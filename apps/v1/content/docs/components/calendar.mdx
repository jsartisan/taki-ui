...
```bash
npx taki-ui@latest add calendar
```
...
After upgrading the `Calendar` component, you can install the new blocks by running the `taki-ui@latest add` command.

```bash
npx taki-ui@latest add calendar-02
```
