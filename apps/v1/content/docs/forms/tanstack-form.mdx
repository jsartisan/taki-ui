...
<ComponentSource
  src="/registry/new-york/examples/form-tanstack-demo.tsx"
  title="form.tsx"
/>
...
- Add the `aria-invalid` or `isInvalid` prop to the form control such as `<Input />`, `<Select />`, `<Checkbox />`, etc.
...
- For select components, use `field.state.value` with `selectedKey` and `field.handleChange` with `onSelectionChange` on the `<Select />` component.
...
```tsx showLineNumbers title="form.tsx" {6,17-18}
<form.Field
  name="language"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
