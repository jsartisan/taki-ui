...
<ComponentSource
  src="/registry/v1/examples/form-rhf-demo.tsx"
  title="form.tsx"
/>
...
- For select components, use `field.value` with `selectedKey` and `field.onChange` with `onSelectionChange` on the `<Select />` component.
...
```tsx showLineNumbers title="form.tsx" {5,13,17-18}
<Controller
  name="language"
  control={form.control}
  render={({ field, fieldState }) => (
